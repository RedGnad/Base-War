...
- Light is the scene's job, not the model's. Rare items and above, and anything mutated, get a
  point light in their colour under the model (`lumiereDuJouet` in `src/client/toy.ts`), and
  the turret gets one while it holds charges. Do not bake glow into the mesh; an emissive
  texture in the GLB is welcome and the light will pick up its colour on the slab.
- Image textures the world animates live in `assets/textures/` and are generated by
  `tools/world/build-world-textures.py`, so a change to a pattern is a change to that script.
